...
bool bfs(size_t const node_count, bool graph[node_count][node_count], size_t start, size_t needle) {
    // has this node been searched?
    bool seen[node_count];
    memset(seen, false, node_count); // (defaults all elements to 0, which is false)

    // there can be max (node_count) nodes in the queue, if we do not re-queue them
 ...
 */
void spanning_tree(size_t node_count, bool graph[node_count][node_count], size_t parent[node_count]) {
    // has this node been searched?
    bool seen[node_count];
    memset(seen, false, node_count); // (defaults all elements to 0, which is false)
