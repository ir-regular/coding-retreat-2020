...
void matrix_mult(size_t const x, size_t const y, size_t const shared,
		 double matrix_a[x][shared], double matrix_b[shared][y],
		 double result[x][y])
{
	for (size_t j = 0; j < y; j++) {
		...
		for (size_t i = 0; i < x; i++) {
			result[i][j] =
				dot_product(shared, matrix_a[i], column_b);
		}
	}
}
